...
-(id)initWithQuery:(NSString*)queryString NumberOfResults:(int)numberOfResults Radius:(float)radius Coordinate:(CLLocationCoordinate2D)coordinate{
  NSURL * url = [NSURL URLWithString:[NSString stringWithFormat:@"http://local.yahooapis.com/LocalSearchService/V3/localSearch?query=%@&results=%d&radius=%f&latitude=%f&longitude=%f&appid=%@",queryString,numberOfResults,radius,coordinate.latitude, coordinate.longitude,YAHOO_APP_ID]];
